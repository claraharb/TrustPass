...
export async function analyzeOtpActivity(
  ...
  const recentRequests = await prisma.trustRequest.count({
    where: {
      clientId,
      phoneNumber,
      protectedAction: {
  ...
  // The current request is not yet included in the database when
  // this function runs, so add it to the recent count. If the
  // caller explicitly provides an attemptCount, use the larger
  // value so the API can represent the client's observed number
  // of attempts.
  const totalAttempts = Math.max(recentRequests + 1, attemptCount ?? 1);

  let isBombing: boolean;
  ...
  if (totalAttempts >= 10) {
    ...
    // Suspicious, but not strong enough to classify as full OTP
    // bombing.
    isBombing = false;
    riskScore = 40;
    details =
