...
const completedVerifications = new Map<string, CompletedVerification>();
...
function getNumberVerificationConfig() {
  ...
  const authorizationUrl = process.env.NOKIA_NV_AUTHORIZATION_URL;
  ...
  if (
    ...
    throw new Error("Nokia Number Verification configuration is missing");
  }

  return { clientId, clientSecret, redirectUri, authorizationUrl, tokenUrl };
}
...
/**
 * Demo bypass for Number Verification.
 *
...
export function createNumberVerificationAuthorizationUrl(
  ...
  const { clientId, redirectUri, authorizationUrl } =
 ...
 */
function getVerificationSession(state: string) {
  const session = verificationSessions.get(state);

  if (!session) {
    throw new Error("Invalid or expired Number Verification state");
  }

  if (Date.now() - session.createdAt > SESSION_EXPIRATION_MS) {
    verificationSessions.delete(state);
    throw new Error("Number Verification session has expired");
  }
...
/**
 * Exchanges the Nokia authorization code for an access token.
 */
async function exchangeAuthorizationCode(
  code: string
): Promise<NumberVerificationTokenResponse> {
  const { clientId, clientSecret, tokenUrl } = getNumberVerificationConfig();
  ...
  if (!data.access_token || typeof data.access_token !== "string") {
    throw new Error("Invalid access token response from Nokia");
  }
...
async function verifyPhoneNumber(
  ...
  const data = (await response.json()) as Partial<NumberVerificationResult>;

  if (typeof data.devicePhoneNumberVerified !== "boolean") {
    throw new Error("Invalid Number Verification response from Nokia");
  }

  return { devicePhoneNumberVerified: data.devicePhoneNumberVerified };
}
...
export async function handleNumberVerificationCallback(
  ...
  // Check whether this state has already completed. This prevents
  // duplicate browser/network callbacks from appearing as failed
  // verifications.
  const completed = completedVerifications.get(state);

  if (completed) {
    if (Date.now() - completed.completedAt <= COMPLETED_RESULT_EXPIRATION_MS) {
      return {
        phoneNumber: completed.phoneNumber,
        trustRequestId: completed.trustRequestId,
  ...
  const session = getVerificationSession(state);
  ...
  try {
    ...
    const completedResult = {
      ...
      trustRequestId: session.trustRequestId,
      verified: result.devicePhoneNumberVerified,
      completedAt: Date.now(),
    };

    // Save the completed result temporarily so duplicate callbacks
    // can return the same answer.
    completedVerifications.set(state, completedResult);

    return {
      phoneNumber: completedResult.phoneNumber,
      trustRequestId: completedResult.trustRequestId,
      verified: completedResult.verified,
    };
  } finally {
    // The OAuth state is no longer needed after the verification
    // has completed.
    verificationSessions.delete(state);
  }
}
