...
function getNokiaConfig() {
  const apiKey =
    process.env.NOKIA_RAPIDAPI_KEY;

  const host =
    process.env.NOKIA_RAPIDAPI_HOST;
...
export async function checkSimSwap(
  ...
  const url =
    process.env.NOKIA_SIM_SWAP_URL;

  if (!url) {
    throw new Error(
      "NOKIA_SIM_SWAP_URL is not defined"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Invalid SIM Swap response from Nokia"
    );
  }
...
export async function checkDeviceSwap(
  ...
  const url =
    process.env.NOKIA_DEVICE_SWAP_URL;

  if (!url) {
    throw new Error(
      "NOKIA_DEVICE_SWAP_URL is not defined"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Invalid Device Swap response from Nokia"
    );
  }
...
export async function checkDeviceReachability(
  ...
  const url =
    process.env.NOKIA_DEVICE_STATUS_URL;

  if (!url) {
    throw new Error(
      "NOKIA_DEVICE_STATUS_URL is not defined"
    );
  }
  ...
  if (
    typeof data.reachable !==
    "boolean"
  ) {
    throw new Error(
      "Invalid Device Status response from Nokia"
    );
  }
